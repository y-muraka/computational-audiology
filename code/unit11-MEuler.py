...
def modified_euler_method(m, c, k, F0, omega, x0, v0, dt, t_end):
    """
    Numerical solution of a single-degree-of-freedom oscillation system using Modified Euler method
    ...
    # Time integration using Modified Euler method
    for i in range(1, n_steps):
        # Predictor step (Euler method)
        x_pred = x[i-1] + v[i-1] * dt
        v_pred = v[i-1] + a[i-1] * dt
        
        # Calculate acceleration at predicted state
        a_pred = (F0 * np.cos(omega * t[i]) - c * v_pred - k * x_pred) / m
        
        # Corrector step (average of slopes)
        x[i] = x[i-1] + 0.5 * (v[i-1] + v_pred) * dt
        v[i] = v[i-1] + 0.5 * (a[i-1] + a_pred) * dt
        
        # Calculate acceleration at corrected state
...
def main():
    ...
    t, x_euler, v_euler = modified_euler_method(m, c, k, F0, omega, x0, v0, dt, t_end)
   ...
   # Calculate error
    ...
    plt.savefig('waveform-MEuler.pdf')
    plt.show()
    ...
    plt.loglog(dt_values, [dt**2 for dt in dt_values], '--', label='Second Order Slope', c='tab:orange')
